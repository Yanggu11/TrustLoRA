...
class DynamicLoRALayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.matmul(torch.matmul(x, B), A)  # returns [batch, seq_len, hidden]
...
dynamic_lora_layer = DynamicLoRALayer(base_model.hidden_size, base_model.lora_r, base_model.hypernet)
...
print(base_model.roberta.encoder.layer[-1])
