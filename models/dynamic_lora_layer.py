...
class DynamicLoRALayer(nn.Module):
    def __init__(self, hidden_size: int, r: int, hypernet: nn.Module, layer_id: int, hypernet_use_batches: bool = False, initial_A: torch.Tensor = None, initial_B: torch.Tensor = None, noise_type_A: str = "replace", noise_type_B: str = "replace", noise_alpha: float = 0.5):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.hypernet_use_batches:
            A_hypernet, B_hypernet = self.hypernet(self.layer_id, device=x.device)  # A: [hidden, r], B: [r, hidden]
        else:
            A_hypernet, B_hypernet = self.hypernet.use_precomputed(self.layer_id) # A: [hidden, r], B: [r, hidden]
