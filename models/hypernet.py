...
class LoRAHyperNet(nn.Module):
    def __init__(
        ...
        self.lora_r = lora_r
        self.hidden_dim = (
            hidden_dim if isinstance(hidden_dim, list) else [hidden_dim] * 3
        )
        ...
        assert not (
            not self.embedding_input_only and self.hypernet_A_matrix == "generated"
    ...
    def precompute(self, device="cpu"):
        ...
        self.A_matrices = [
            (
                As[i].detach().clone()
                if not self.hypernet_A_matrix == "generated"
                else As[i].clone()
            )
            for i in range(self.num_of_embeddings)
        ]

        self.B_matrices = [Bs[i].clone() for i in range(self.num_of_embeddings)]
...
class LoRAHyperNetTransformer(nn.Module):
    ...
    def forward(self, layer_id, device="cpu"):
        ...
        raise NotImplementedError(
            "Use precompute() and use_precomputed() methods instead."
        )
    ...
    def precompute(self, device="cpu"):
        ...
        if self.hypernet_A_matrix == "generated":
            self.A_matrices = [
                out[i].view(self.input_dim, self.lora_r).clone()
            ...
            self.B_matrices = [out[i].clone() for i in range(self.num_of_embeddings)]
