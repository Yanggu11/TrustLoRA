...
def ece(probabilities: np.ndarray, labels: np.ndarray, n_bins: int = 10) -> float:
    ...
    for i in range(n_bins):
        ...
        if proportion_in_bin > 0:
            ...
            ece_value += np.abs(avg_confidence_in_bin - accuracy_in_bin) * proportion_in_bin
    
    return ece_value

def classwise_ece(probabilities: np.ndarray, labels: np.ndarray, n_bins: int = 10) -> float:
    """
    ...
    for c in range(n_classes):
        ...
        for i in range(n_bins):
            ...
            if i == 0:
                in_bin_mask = (class_confidences >= bin_lower) & (class_confidences <= bin_upper)
            else:
                in_bin_mask = (class_confidences > bin_lower) & (class_confidences <= bin_upper)
            ...
            if n_in_bin > 0:
                ...
                cece_value += (n_in_bin / (n_samples * n_classes)) * np.abs(accuracy_in_bin - avg_confidence_in_bin)
...
def thresholded_ece(probabilities: np.ndarray, labels: np.ndarray, threshold: float = 0.001, n_bins: int = 10) -> float:
    """
    Calculate the Thresholded Expected Calibration Error (Thresholded ECE).
    
    This variant of ECE only evaluates probabilities that are above a specified threshold,
    filtering out infinitesimal softmax predictions that can wash out the calibration score.
    This is particularly useful for multi-class problems where many predictions correspond
    to incorrect classes with very small probabilities.
    ...
    Returns:
        The Thresholded Expected Calibration Error as a float in the range [0, 1].
        Lower values indicate better calibration.
    
    Example:
        >>> probs = np.array([[0.9, 0.1], [0.6, 0.4], [0.3, 0.7]])
        >>> labels = np.array([0, 0, 1])
        >>> tece_score = thresholded_ece(probs, labels, threshold=0.001, n_bins=10)
    """
    n_samples, n_classes = probabilities.shape
    ...
    for i in range(n_bins):
        ...
        if i == 0:
            in_bin_mask = (filtered_probs >= bin_lower) & (filtered_probs <= bin_upper)
        else:
            in_bin_mask = (filtered_probs > bin_lower) & (filtered_probs <= bin_upper)
        ...
        if proportion_in_bin > 0:
            accuracy_in_bin = np.mean(accuracies[in_bin_mask])
            avg_confidence_in_bin = np.mean(filtered_probs[in_bin_mask])
            ece_value += np.abs(avg_confidence_in_bin - accuracy_in_bin) * proportion_in_bin
    
    return ece_value

def thresholded_ace(probabilities: np.ndarray, labels: np.ndarray, threshold: float = 0.001, n_bins: int = 10) -> float:
    """
